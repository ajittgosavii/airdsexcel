...
def parse_uploaded_file(uploaded_file):
    ...
    df.rename(columns={k: v for k, v in required_columns.items()}, inplace=True)
    ...
    if 'database_name' in df.columns:
        df.rename(columns={'database_name': 'db_name'}, inplace=True)
    else:
        df['db_name'] = [f"Database {i+1}" for i in range(len(df))]
    
    # Convert to list of dictionaries
    inputs_list = df.to_dict(orient='records')
    ...
    for idx, input_data in enumerate(inputs_list):
        ...
        if not input_errors:
            ...
            errors.append(f"{db_name}: {', '.join(input_errors)}")
...
def export_full_report(all_results):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for result in all_results:
            db_name = result['inputs'].get('db_name', 'Database')
            sheet_name = db_name[:25] + '...' if len(db_name) > 28 else db_name
            
            # Prepare detailed data
            details = {
                "Input Parameters": [json.dumps(result['inputs'], indent=2)],
                "Recommendations": [
                    "\n".join([f"{env}: {rec['instance_type']} (${rec['monthly_cost']:.2f}/mo)" 
            ...
            if result.get('ai_insights'):
                details["AI Insights"] = [json.dumps(result['ai_insights'], indent=2)]
            
            pd.DataFrame(details).to_excel(writer, sheet_name=sheet_name, index=False)
